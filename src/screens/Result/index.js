...
class Result extends Component {
    ...
    componentDidMount(){
        ...
        if(detectionResult){ // if there is result 
            ...
            first.map(second => { // second depth
                ...
                string = string.split(" "); // split texts by space
                    for(let i =0; i < string.length; i++){
                      if(string[i].length > 0 && string[i].length != ""){ // if it's not [""] push it to the stringArray
                        stringArray.push(string[i].toLowerCase());
                      }
                  }
            mockData.map((item)=>{ // match with database
              // if additive exist, not scanned twice, and match database
              if ( item.name && !touched.includes(item.name) && (item.name.toLowerCase().includes(stringArray) || stringArray.indexOf(item.name) > -1)){
        ...
        return(
            <Container>
              <Header style={styles.header}>
                <Left>
                  <Icon name="x" onPress={() => this.props.navigation.goBack()} style={styles.icon}/>
                </Left>
              </Header>
                <Content>
                    <FlatList
